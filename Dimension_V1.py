...
try:
    ...
    #print(metricL)
    print(top[1], bottom[1], top[0], bottom[0])
    theta = math.atan((top[1]-bottom[1])/(top[0]-bottom[0]))
    theta2 = math.atan((right[1]-left[1])/(right[0]-left[0]))
    newY = int(bottom[1]+5)
    newX = int(bottom[0]-(-5/math.tan(theta)))

    print("bottom: ", newX, newY)
    bottom_ext = np.linspace(bottom, (newX,newY), 5, dtype = int)
    bottomExtCoord = np.zeros([5, 3])
    print(bottom_ext)
    i = 0
    for points in bottom_ext:
        bottomExtCoord[i] = convert_depth_pixel_to_metric_coordinate(depth_frame.get_distance(points[0], points[1]), points[0], points[1], intrinsics)
        i+=1

    newY = int(top[1]-5)
    newX = int(top[0]-(5/math.tan(theta)))
    print("top:", newX, newY)
    top_ext = np.linspace(top, (newX,newY), 5, dtype = int)
    topExtCoord = np.zeros([5, 3])
    print(top_ext)
    i = 0
    for points in top_ext:
        topExtCoord[i] = convert_depth_pixel_to_metric_coordinate(depth_frame.get_distance(points[0], points[1]), points[0], points[1], intrinsics)
        i+=1

    newY = int(right[1]-(-5*math.tan(theta2)))
    newX = int(right[0]+5)
    print("right: ", newX, newY)
    right_ext = np.linspace(right, (newX,newY), 5, dtype = int)
    rightExtCoord = np.zeros([5, 3])
    print(right_ext)
    i = 0
    for points in right_ext:
        rightExtCoord[i] = convert_depth_pixel_to_metric_coordinate(depth_frame.get_distance(points[0], points[1]), points[0], points[1], intrinsics)
        i+=1

    newY = int(left[1]-(5*math.tan(theta2)))
    newX = int(left[0]-5)
    print("Left: ", newX, newY)
    left_ext = np.linspace(left, (newX,newY), 5, dtype = int)
    leftExtCoord = np.zeros([5, 3])
    print(left_ext)
    i = 0
    for points in left_ext:
        leftExtCoord[i] = convert_depth_pixel_to_metric_coordinate(depth_frame.get_distance(points[0], points[1]), points[0], points[1], intrinsics)
        i+=1

    right_zero = np.mean(rightExtCoord[:,2])
    ...
    for i in range(1,11):
        if Lfinal10[-i,2] > 5:
            if percent_error(Lfinal10[-i,1]) < 0.5:
    ...
    for i in range(1,11):
        if Wfinal10[-i,2] > 5:
            if (Wfinal10[-i,1] > zero_min) and (Wfinal10[-i,1] < zero_max):
